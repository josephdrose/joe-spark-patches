...
# Shared </think>-leak filter. The reasoning parser strips a stray </think> while
# IN the reasoning phase, but once reasoning ends (first </think>) the serving
# orchestrator (vllm parse_delta) routes content-phase deltas through THIS tool
# parser instead — so a stray SECOND </think> that arrives after reasoning-end
# only passes through here. Mounted next to the reasoning parser at
# vllm/reasoning/dsv4_dsml_split.py; flat fallback for standalone/test import.
try:
    from vllm.reasoning.dsv4_dsml_split import filter_think_close as _filter_think_close
except ImportError:  # pragma: no cover - standalone import fallback
    from dsv4_dsml_split import filter_think_close as _filter_think_close  # type: ignore

logger = init_logger(__name__)

# DSML_TRACE=1: dump per-delta tool-parser state to stderr (interleaves with the
# reasoning parser's [DSML-r] trace) to debug the reasoning->tool handoff leak.
_DSML_TRACE = os.environ.get("DSML_TRACE", "0") == "1"
_dsml_t_n = [0]


def _tlog(msg: str) -> None:
    if _DSML_TRACE and _dsml_t_n[0] < 400:
        _dsml_t_n[0] += 1
...
class DeepSeekV32ToolParser(ToolParser):
    ...
    def __init__(self, tokenizer: TokenizerLike, tools: list[Tool] | None = None):
        ...
        # Tool-call BODY marker — the model emits it ONLY inside a tool-calls
        # block, so it's an unambiguous signal that a tool call has opened even
        # when the start tag itself was corrupted by spec-decode (see the
        # opener-truncation recovery in _process_streaming_buffer).
        self.invoke_marker = "<｜DSML｜invoke"
    ...
    def extract_tool_calls(
        ...
        # MTP REPAIR: spec-decode can drop the start tag's leading '<' (see
        # _process_streaming_buffer); splice it back. Only the first marker is
        # ever headless, and ｜DSML｜…> only ever opens a tool call.
        headless = self.tool_call_start_token[1:]
        if (
            self.tool_call_start_token not in model_output
            and headless in model_output
        ):
    ...
    def _reset_streaming_state(self):
        ...
        self._buffer = ""
        # Withheld trailing partial </think> for the content-channel leak filter.
        self._think_pending = ""
    ...
    def _process_streaming_buffer(
        ...
        while True:
            if not self._in_tool_calls:
                ...
                if start_idx == -1:
                    # OPENER-TRUNCATION recovery (DSpark): spec-decode can drop a
                    # variable-length CHUNK of the start tag at a draft-rejection
                    # boundary — e.g. "<｜DSML｜tool_c" vanishes, leaving "alls>"
                    # which find() can't match; the reasoning->tool handoff then
                    # hands the tool parser only that tail with previous_text=""
                    # (buffer reset), so the dropped head is gone for good
                    # (DSML_TRACE 2026-06-28: reset=1 prev='' delta=
                    # 'alls>\n<｜DSML｜invoke'). The invoke body marker is emitted
                    # ONLY inside a tool-calls block, so its presence with NO valid
                    # start tag means the opener was corrupted: drop the dead
                    # fragment and open the block at the invoke. Checked FIRST so it
                    # also beats a spurious headless match on the END tag
                    # </｜DSML｜tool_calls>. DSpark variant of the MTP headless leak
                    # (vLLM #36654 / #41132).
                    inv_idx = self._buffer.find(self.invoke_marker)
                    if inv_idx != -1:
                        self._buffer = self._buffer[inv_idx:]
                        self._in_tool_calls = True
                        continue
                    # MTP REPAIR: spec-decode intermittently drops ONLY the leading
                    # '<' of the start tag at a draft-rejection boundary (model
                    # jumps straight to a tool call), leaving a headless
                    # ｜DSML｜tool_calls> that won't match. The model never emits
                    # ｜DSML｜…> except to open a tool call, so once the headless
                    # tag is complete, splice the '<' back in. Exclude a '<'- or
                    # '/'-preceded match (an intact start tag or the END tag).
                    # 2026-06-14, mirrors the repair in dsv4_dsml_split.py.
                    headless = self.tool_call_start_token[1:]
                    h_idx = self._buffer.find(headless)
                    if h_idx != -1 and (
                        h_idx == 0 or self._buffer[h_idx - 1] not in ("<", "/")
                    ):
                        self._buffer = (
                            self._buffer[:h_idx] + "<" + self._buffer[h_idx:]
                        )
                        start_idx = h_idx
                    else:
                        # Truncated-opener TAIL: spec-decode can drop the HEAD of
                        # the start tag (not just the leading '<'), leaving a bare
                        # SUFFIX like "tool_calls>" that arrives in its own delta
                        # BEFORE the invoke (DSML_TRACE 2026-06-28: a stray
                        # "tool_calls>\n" leaked while the tool calls still fired).
                        # If the buffer minus trailing whitespace is exactly a
                        # start-tag suffix, hold the WHOLE buffer — the next delta
                        # either completes it into an invoke-recovery above or, on
                        # a reset, drops it (no flush); a non-tool continuation or
                        # EOS flushes it. >=4 chars skips trivial ">"/"s>" tails.
                        st = self.tool_call_start_token
                        # Strip a trailing partial of the invoke marker first (the
                        # body opener may have started arriving right after the
                        # tail), then test whether what remains is a bare start-tag
                        # suffix — i.e. the buffer is [tag-suffix][ws][invoke?].
                        inv_ov = partial_tag_overlap(self._buffer, self.invoke_marker)
                        pre = self._buffer[: len(self._buffer) - inv_ov].rstrip()
                        if 4 <= len(pre) < len(st) and st.endswith(pre):
                            return
                        # Withhold a partial start tag / headless tag / invoke
                        # marker still arriving across chunks; flush only what
                        # cannot be the beginning of one.
                        overlap = max(
                            partial_tag_overlap(self._buffer, st),
                            partial_tag_overlap(self._buffer, headless),
                            partial_tag_overlap(self._buffer, self.invoke_marker),
                        )
    ...
    def extract_tool_calls_streaming(
        ...
        if _DSML_TRACE:
            _tlog(f"[DSML-t] reset={int(did_reset)} prev=...{previous_text[-16:]!r} "
                  f"delta={delta_text!r} bufIn={buf_in[:40]!r} "
                  f"in_tc={int(self._in_tool_calls)} -> content={content_parts!r} "
                  f"ntc={len(tool_call_deltas)} bufOut={self._buffer[:32]!r}")

        if content_parts or tool_call_deltas:
            content = "".join(content_parts)
            if content:
                content, self._think_pending = _filter_think_close(
                    self._think_pending, content
                )
            return DeltaMessage(
                content=content or None, tool_calls=list(tool_call_deltas.values())
            )

        # Empty delta with token ids means EOS or closing tag.
        if not delta_text and delta_token_ids:
            # Flush any withheld non-tool buffer (a start-tag suffix held by the
            # truncated-opener withhold that never resolved into a tool call) so
            # trailing content isn't lost at stream end.
            if not self._in_tool_calls and self._buffer:
                flushed, self._buffer = self._buffer, ""
                flushed, self._think_pending = _filter_think_close(
                    self._think_pending, flushed
                )
                return DeltaMessage(content=flushed or None)
            # Otherwise return non-None so the framework can finalize
            # finish_reason.
