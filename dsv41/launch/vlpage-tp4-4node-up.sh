#!/usr/bin/env bash
# vlpage-tp4-4node-up.sh — DeepSeek-V4.1-Flash on the 4-Spark switched mesh under vLLM
# PR #56214, with the Engram table on NVMe (engram-disk-table.patch).
#
# IMAGE vlpage-eng:3 is built by build/vl41-build-image.sh then
# build/vlpage-build-image.sh. Layers, in order:
...
#
# Usage:  vlpage-tp4-4node-up.sh            # bring up
#         DRYRUN=1 vlpage-tp4-4node-up.sh   # print per-rank scripts, change nothing
set -uo pipefail

IMAGE="${IMAGE:-vlpage-eng:3}"
...
EAGER="${EAGER:-1}"                 # 1 = --enforce-eager, skips graph capture
...
sshpipe() { ssh $SSH_OPTS "${USER}@${NODE_TS[$1]}" 'cat > /tmp/vlpage-launch.sh && bash /tmp/vlpage-launch.sh'; }
...
EAGER_ARG=""; [ "$EAGER" = 1 ] && EAGER_ARG="--enforce-eager"
SPEC_ARG=""
[ -n "$DSPARK" ] && SPEC_ARG='--speculative-config "{\"method\":\"dspark\",\"num_speculative_tokens\":'"$DSPARK}\""
...
docker run -d --name $NAME --network host --ipc host --shm-size 32g --gpus all \\
  ...
  --entrypoint /bin/bash $IMAGE -lc '
    exec vllm serve $SNAP \\
      ...
      $EAGER_ARG --enable-chunked-prefill \\
...
if [ "${DRYRUN:-0}" = 1 ]; then
  echo "### DRYRUN IMAGE=$IMAGE TP=$TP CTX=$CTX UTIL=$GPU_UTIL EAGER=$EAGER"
  for r in 0 1 2 3; do echo "===== rank $r -> ${NODE_TS[${R2I[$r]}]} ====="; runscript "$r"; echo; done
  exit 0
fi

echo ">>> [1/3] clearing $NAME on all 4 boxes"
